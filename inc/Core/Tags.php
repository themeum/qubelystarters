...
class Tags {
	...
	public static function posted_on() {
		...
		$categories_list = get_the_category_list( esc_html__( ', ', 'qubelystarters' ) );
		$categories      = sprintf(
			// Translators: translatable string for post categories.
			'<span class="cat-links blog-cats">' . esc_html_x( ' in %1$s', 'post categories', 'qubelystarters' ) . '</span>',
			$categories_list
		); // WPCS: XSS OK.

		$posted_on = sprintf(
			// Translators: translatable string for post date.
			esc_html_x( 'Posted on %s', 'post date', 'qubelystarters' ),
			'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
		);
		$byline = sprintf(
			// Translators: translatable string for post author.
			esc_html_x( 'by %s', 'post author', 'qubelystarters' ),
	...
	public static function entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list && self::categorized_blog() ) {
				// Translators: translatable string for post categories.
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'qubelystarters' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}
			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html__( ', ', 'qubelystarters' ) );
			if ( $tags_list ) {
				// Translators: translatable string for post tags.
				printf( '<span class="tags-links">' . esc_html__( ' Tagged %1$s', 'qubelystarters' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}
		if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			echo '<span class="comments-link">';
			// Translators: %s: post title.
			comments_popup_link( sprintf( wp_kses( __( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'qubelystarters' ), array( 'span' => array( 'class' => array() ) ) ), get_the_title() ) );
			echo '</span>';
		}
		edit_post_link(
			sprintf(
				// Translators: %s: Name of current post.
				esc_html__( ' Edit %s', 'qubelystarters' ),
